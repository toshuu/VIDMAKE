/** Render the X80 AI voice-calling reel: preview stills + full 15s H.264 MP4. */
...
import {
  buildVoiceReel, FONT, DISP, GRAIN, DUR,
  IMG_HOOK, IMG_INSTANT, IMG_LANG, IMG_SCALE, IMG_SUCCESS, IMG_DEAL,
} from './plan-voice.mjs';

const DIR = dirname(fileURLToPath(import.meta.url));
const OUT = join(DIR, '../../output/voice-x80');
...
const VOICE = '/kaggle/working/my-video/public/voice';
const entries = {
  [IMG_HOOK]: join(VOICE, 'hook.jpg'),
  [IMG_INSTANT]: join(VOICE, 'instant.jpg'),
  [IMG_LANG]: join(VOICE, 'languages.jpg'),
  [IMG_SCALE]: join(VOICE, 'scale.jpg'),
  [IMG_SUCCESS]: join(VOICE, 'success.jpg'),
  [IMG_DEAL]: join(VOICE, 'deal.jpg'),
};
...
const grain = createGrainTile(660, 1080, { seed: 47, amount: 0.5 });

const renderer = new SkiaRenderer();
const measure = createSkiaMeasurer();
const plan = buildVoiceReel(measure);
const options = () => ({
  measureText: measure,
...
// Audio: low whoosh bed + stingers on the 4 cuts.
...
writeFileSync(join(OUT, 'voice-x80-15s.mp4'), mp4);
