/** Render INDIA FEST: ChatGPT-planned dossier, engine-built. Stills + 15s MP4. */
...
import { buildIndiaFest } from './plan-india.mjs';

const DIR = dirname(fileURLToPath(import.meta.url));
const OUT = join(DIR, '../../output/india-fest');
...
const NAMES = ['lehenga', 'kurta', 'sari', 'mundu'];
const entries = {};
for (const n of NAMES) {
  for (let f = 0; f < 5; f += 1) {
    entries[`in2-${n}-${f}`] = join(DIR, `sprites/2x/${n}-${f}.png`);
    entries[`in1-${n}-${f}`] = join(DIR, `sprites/1x/${n}-${f}.png`);
  }
}
for (const n of ['diya', 'lantern']) {
  entries[`prop-${n}`] = join(DIR, `sprites/props2x/${n}.png`);
  entries[`prop1-${n}`] = join(DIR, `sprites/props/${n}.png`);
}
...
const plan = buildIndiaFest(measureFn);
...
for (const act of plan.composition.root.children) {
  if (act.id !== 'chrome' && act.children) {
    act.children.push({
      id: `${act.id}-grain`, type: 'image', src: 'grain-tile',
...
writeFileSync(join(OUT, 'india-fest-15s.mp4'), mp4);
