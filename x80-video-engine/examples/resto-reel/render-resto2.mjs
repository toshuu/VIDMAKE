...
import {
  buildVrindavan2, CLIP_SIGN, CLIP_BUFFET, CLIP_TABLES, CLIP_ENTRY, CLIP_GODS,
} from './plan-resto2.mjs';
import { GRAIN } from './plan-resto.mjs';

const DIR = dirname(fileURLToPath(import.meta.url));
const NOCHIPS = process.argv.includes('nochips');
const OUT = join(DIR, '../../output/vrindavan2');
...
const ICONS = NOCHIPS ? [] : ['cloche', 'chef', 'cocktail'];
const entries = {};
...
const plan = buildVrindavan2(measureFn, { chips: !NOCHIPS });
...
writeFileSync(join(OUT, NOCHIPS ? 'vrindavan-nochips-15s.mp4' : 'vrindavan2-15s.mp4'), mp4);
