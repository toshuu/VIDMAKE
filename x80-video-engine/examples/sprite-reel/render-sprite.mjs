/** Render THE SPRITE PARADE: preload 3x/2x sprite sets, stills + 15s MP4. */
...
import { buildSpriteParade } from './plan-sprite.mjs';

const DIR = dirname(fileURLToPath(import.meta.url));
const OUT = join(DIR, '../../output/sprite-parade');
...
const NAMES = ['wizard', 'girl', 'bird', 'pirate', 'robot'];
const entries = {};
for (const n of NAMES) {
  for (let f = 0; f < 5; f += 1) {
    entries[`spr3-${n}-${f}`] = join(DIR, `sprites/3x/${n}-${f}.png`);
    entries[`spr2-${n}-${f}`] = join(DIR, `sprites/2x/${n}-${f}.png`);
  }
}
...
const plan = buildSpriteParade(measureFn);
...
writeFileSync(join(OUT, 'sprite-parade-15s.mp4'), mp4);
